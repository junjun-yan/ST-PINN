...
class PhysicsInformedNN:
    ...
    def __init__(self, x_init, t_init, u_init, x_l_bound, x_r_bound, t_bound, x_eqns, t_eqns, u_eqns,
        ...
        self.x_eqns, self.t_eqns, self.u_eqns = x_eqns, t_eqns, u_eqns
        ...
        self.x_l_bound_tf, self.x_r_bound_tf, self.t_bound_tf = [tf.placeholder(tf.float32, shape=[None, 1]) for _ in
                                                                 range(3)]
        ...
        self.loss = self.init_loss + self.eqns_loss + self.bound_loss
    ...
    def train(self, max_time, adam_it):
        ...
        while self.it < adam_it and self.total_time < max_time:
            ...
            # evaluate
            if self.it % 100 == 0:
                u_pred = model.predict(self.x_test, self.t_test)
...
if __name__ == '__main__':
    ...
    log_path = "../output/log/burgers1D-pinn-%s" % (create_date)
