...
class PhysicsInformedNN:
    ...
    def __init__(self, x_init, t_init, u_init, x_l_bound, x_r_bound, t_bound, x_sample, t_sample, x_data, t_data,
        ...
        self.loss = self.init_loss + self.eqns_loss + self.data_loss + self.bound_loss + self.pseudo_loss
    ...
    def callback(self, loss, init_loss, bound_loss, eqns_loss, data_loss, pseudo_loss):
        if self.it % 10 == 0:
            elapsed = time.time() - self.start_time
            self.total_time += elapsed / 3600.0
            log_item = 'It: %d, Loss: %.3e, Init Loss: %.3e, Bound Loss: %.3e, Eqns Loss: %.3e, Data Loss: %.3e, ' \
                       'Pseudo Loss: %.3e, Time: %.2fs, Total Time: %.2fh' % \
                       (self.it, loss, init_loss, bound_loss, eqns_loss, data_loss, pseudo_loss, elapsed,
                        self.total_time)
            self.logging(log_item)
            self.start_time = time.time()
        self.it += 1
